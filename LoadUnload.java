        ...
        CopyOnWriteArrayList<Movie> movieList = new CopyOnWriteArrayList<>();
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (data.length == 4) {
                        // Assuming the format is: Title, Actor, Genre, Year
                        String title = data[0].trim();
                        String actor = data[1].trim();
                        String genre = data[2].trim();
                        String year = data[3].trim();
    
                        Movie movie = new Movie(title, actor, genre, year);
                        movieList.add(movie);
                    }
                }
    ...
    public static void saveFile(CopyOnWriteArrayList<Movie> movieList){
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filePath))) {
            ...
            for (Movie obj : movieList) {
                writer.println(obj.getTitle() + "," + obj.getActor() + "," + obj.getGenre() + "," + obj.getYear()); // Customize this line to fit your object structure, Done
            }
